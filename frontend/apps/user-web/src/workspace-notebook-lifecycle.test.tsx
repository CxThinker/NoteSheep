...
describe("Workspace notebook lifecycle", () => {
  ...
  it("deletes, restores, and permanently deletes a notebook from the sidebar", async () => {
    ...
    const deleteDialog = screen.getByRole("dialog", { name: "删除笔记本" });
    expect(deleteDialog).toHaveTextContent("确定要删除笔记本 笔记本1 吗？可在已删除笔记本中恢复。");
    expect(api.deleteNotebook).not.toHaveBeenCalled();
    fireEvent.click(within(deleteDialog).getByRole("button", { name: "删除笔记本" }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "删除笔记本 笔记本1" }));
    fireEvent.click(within(screen.getByRole("dialog", { name: "删除笔记本" })).getByRole("button", { name: "删除笔记本" }));
    fireEvent.click(screen.getByRole("tab", { name: "已删除笔记本" }));
    ...
    const permanentDeleteDialog = screen.getByRole("dialog", { name: "彻底删除笔记本" });
    expect(permanentDeleteDialog).toHaveTextContent("确定要彻底删除笔记本 笔记本1 吗？此操作不可恢复。");
    fireEvent.click(within(permanentDeleteDialog).getByRole("button", { name: "彻底删除笔记本" }));
